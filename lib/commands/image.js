...
var Image = { 
  ...
  _imagecutCb: function (opts, hash, position, result) {
   ...
   _doImagecut: function (path, element, rect, cb) {
    ...
    this._parseImage(path).then(function (image) {
      ...
      this._saveImage(image.path, pngDst);
  ...
  _imagecompareCb: function (image_a, image_b, hash) {
    var deferred = Q.defer(); 
    console.log('_imagecompareCb 1');
    var cb = function (result, diffPath) {
      console.log('_imagecompareCb 2');
...
console.log('_doImagecompare 1');
    ...
    .then(function (images) {
      ...
      if (!result) {
        ...
        this._saveImage(imageDiffPath.png, images[1]);  
  ...
  _saveImage : function (filePath, png) {
    png.pack().pipe(fs.createWriteStream(filePath, {flags: 'w'}));
  }
};
