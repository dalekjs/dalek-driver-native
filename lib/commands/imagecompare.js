...
var ImageCompare = { 
    ...
    screenshotelement : function (path, selector, hash) {
      this.actionQueue.push(this.webdriverClient.element.bind(this.webdriverClient, selector));
      
      this.actionQueue.push(this.webdriverClient.location.bind(this.webdriverClient, selector));
      var position = {x:0, y:0};
      
      this.actionQueue.push(function (result){ 
        var value = JSON.parse(result);console.log(value);
        position.x = value.value.x;
        position.y = value.value.y;
       });
      
      this.actionQueue.push(this.webdriverClient.size.bind(this.webdriverClient, selector));
      this.actionQueue.push(this._imagecutCb.bind(this, path, selector, hash, position));
      
      return this;
    },
    _imagecutCb: function (path, element, hash, position, result) {
      ...
      this._imagecut(path, element, rect, cb.bind(this));
